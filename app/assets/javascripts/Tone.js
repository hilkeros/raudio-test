	...
	Module(function (Tone) {
	    ...
	    Tone.Sampler.prototype.triggerAttack = function (note, time, velocity) {
	        ...
	        if (difference !== null) {
	            ...
	            var loopStart = this._sampleMap[closestNote]['loopStart'] || 0;
	            var loopEnd = this._sampleMap[closestNote]['loopEnd'] || buffer.duration;
	            var transpose = this._sampleMap[closestNote]['transpose'] || 0;
	            // play that note
	            var source = new Tone.BufferSource({
	                'buffer': buffer,
	                'playbackRate': Tone.intervalToFrequencyRatio(difference + transpose),
	                'loopStart': loopStart,
	                'loopEnd': loopEnd,
	                'fadeIn': this.attack,
